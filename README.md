...
* **Search & Sort:** Real-time search by title, ingredient, or chef(user). Sort by Date, Title, or Prep Time.
...
### Prerequisites 
Since this application uses JavaScript ES Modules (`<script type="module">`), you cannot simply double-click the `index.html` file to run it33]. Browsers block module requests from the `file://` protocol for security reasons.
...
1. Install the package globally: `npm install -g http-server` 
2. Run the command: `http-server` 
3. Navigate to the provided localhost URL.

---

## Firebase Configuration 
The application is currently set up with a specific Firebase configuration. To use your own database:
1. Go to the [Firebase Console](https://console.firebase.google.com/).
2. Create a new project.
3. Enable **Authentication** (Email/Password provider).
4. Enable **Cloud Firestore**.
   > **Note:** Ensure your Firestore Security Rules allow read/write access for authenticated users.
5. In `index.html`, locate the `firebaseConfig` object.
6. Replace the credentials (apiKey, authDomain, etc.) with your own.

---

## Customization 
* **Theme Colors:** Modify the `:root` variables in the CSS `<style>` block:
  * `--popcat-bg`: Background color 
  * `--popcat-accent`: Primary button and highlight color 
* **App Icon:** Replace `popcat_icon.png` and `popcat_full.png` with your own assets.
